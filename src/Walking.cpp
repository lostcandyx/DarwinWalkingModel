...
Walking::Walking()
{
    ...
    if (!LoadINISettings(&ini,"Walking")) 
        printf("Loading parameters error\n");
...
bool Walking::LoadINISettings(minIni* ini, const std::string &section)
{
    ...
    if((value = ini->getd(section, "x_move_amplitude", INVALID_VALUE)) != INVALID_VALUE)        X_MOVE_AMPLITUDE = value;
    else return false;
    if((value = ini->getd(section, "y_move_amplitude", INVALID_VALUE)) != INVALID_VALUE)        Y_MOVE_AMPLITUDE = value;
    else return false;
    if((value = ini->getd(section, "a_move_amplitude", INVALID_VALUE)) != INVALID_VALUE)        A_MOVE_AMPLITUDE = value;
    else return false;
    return true;
}
...
/*
  Matrix3D
  0   1   2   3      Cz*Cy   Cz*Sy*Sx-Sz*Cx   Cz*Sy*Cx+Sz*Sx   point.X
  4   5   6   7      Sz*Cy   Sz*Sy*Sx+Cz*Cx   Sz*Sy*Cx-Cz*Sx   point.Y
  8   9   10  11      -Sy        Cy*Sx            Cy*Cx        point.Z
  12  13  14  15       0           0                0             1
 */
...
void Walking::update_param_time() //导入时间参数
{
    m_PeriodTime = PERIOD_TIME;
    m_DSP_Ratio = DSP_RATIO;     //两足站立的时间比率（0.1）
    m_SSP_Ratio = 1 - DSP_RATIO; //单足站立的时间比率（0.9）
    //DSP + SSP = 1,这是步态的两种状态

    m_X_Swap_PeriodTime = m_PeriodTime / 2;               //X轴移动的交替时间，应该是指左右两足X方向移动分别占的时间，是周期的一半
    m_X_Move_PeriodTime = m_PeriodTime * m_SSP_Ratio;     //X轴移动的时间，等于单足站立时间（SSP）
    m_Y_Swap_PeriodTime = m_PeriodTime;                   //Y轴移动的交替时间，是全周期
    m_Y_Move_PeriodTime = m_PeriodTime * m_SSP_Ratio;     //Y轴移动的时间，等于单足站立时间（SSP）
    m_Z_Swap_PeriodTime = m_PeriodTime / 2;               //Z轴移动的交替时间，是周期的一半
    m_Z_Move_PeriodTime = m_PeriodTime * m_SSP_Ratio / 2; //Z轴移动的时间，等于单足站立时间的一半（0.5*SSP）
    m_A_Move_PeriodTime = m_PeriodTime * m_SSP_Ratio;     //转动的时间，等于单足站立时间（SSP）

    m_SSP_Time = m_PeriodTime * m_SSP_Ratio;                   //单足站立的时间
    m_SSP_Time_Start_L = (1 - m_SSP_Ratio) * m_PeriodTime / 4; //左腿抬起  （0.025*600 = 15）
    m_SSP_Time_End_L = (1 + m_SSP_Ratio) * m_PeriodTime / 4;   //左腿落下  （0.475*600 = 285）
    m_SSP_Time_Start_R = (3 - m_SSP_Ratio) * m_PeriodTime / 4; //右腿抬起  （0.525*600 = 315）
    m_SSP_Time_End_R = (3 + m_SSP_Ratio) * m_PeriodTime / 4;   //右腿落下  （0.975*600 = 585）

    m_Phase_Time1 = (m_SSP_Time_End_L + m_SSP_Time_Start_L) / 2; //阶段一时间中点  （0.25*600 = 150）
    m_Phase_Time2 = (m_SSP_Time_Start_R + m_SSP_Time_End_L) / 2; //阶段二时间中点  （0.50*600 = 300）
    m_Phase_Time3 = (m_SSP_Time_End_R + m_SSP_Time_Start_R) / 2; //阶段三时间中点  （0.75*600 = 450）

    m_Pelvis_Offset = PELVIS_OFFSET * MX28::RATIO_ANGLE2VALUE; //骨盆补偿  3.0*4096/360
    m_Pelvis_Swing = m_Pelvis_Offset * 0.35;                   //骨盆摇摆补偿
    m_Arm_Swing_Gain = ARM_SWING_GAIN;                         //胳膊摇动增量（待证）
}

void Walking::update_param_move() //导入动作参数
{
    // Forward/Back
...
void Walking::update_param_balance() //加载平衡相关参数
{
    m_X_Offset = X_OFFSET;
    m_Y_Offset = Y_OFFSET;
    m_Z_Offset = Z_OFFSET;
...
void Walking::Process()
{
    ...
    double offset;
    double TIME_UNIT = 8; //MotionModule::TIME_UNIT
    ...
    ep[1] = y_swap + y_move_r - m_Y_Offset / 2 + m_Y_Offset_Haojie;
    ...
    ep[7] = y_swap + y_move_l + m_Y_Offset / 2 + m_Y_Offset_Haojie;
    ...
    // Compute angles
    if ((computeIK(&angle[0], ep[0], ep[1], ep[2], ep[3], ep[4], ep[5]) == 1) && (computeIK(&angle[6], ep[6], ep[7], ep[8], ep[9], ep[10], ep[11]) == 1))
    {
        for (int i = 0; i < 12; i++)
        {
            angle[i] *= 180.0 / PI;
    ...
    PELVIS_OFFSET_L = pelvis_offset_l;//7
    PELVIS_OFFSET_R = pelvis_offset_r;//1
