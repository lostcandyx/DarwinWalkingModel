...
#define WALKING_SECTION "Walking"
...
public:
	...
	bool LoadINISettings(minIni* ini, const std::string &section);
	...
	void ApplyTimeFlow() { m_Time += 8; }
	void SetTime(double x) { m_Time = x; }

	const int PARAMETER_NUMBER = 46;

	double m_Y_Offset_Haojie;

	double PELVIS_OFFSET_R;
	double PELVIS_OFFSET_L;

	double EP[12];
	double ANGLE[12];
	double GetEP(int x) { return EP[x]; }
	double GetANGLE(int x) { return ANGLE[x]; }
};
